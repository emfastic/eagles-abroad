import { Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, HStack, Box, VStack } from '@chakra-ui/react'
...
export default function ProfileTagModal({isOpen, onClose, profile}: any) {
  return ( profile &&
    ...
    <ModalContent pb='3'>
      ...
      <ModalBody>
        <VStack align='left'>
        {profile.instagram ? <HStack spacing="4">
          <InstaIcon boxSize="2em" />
          <Box>{profile.instagram}</Box>
        </HStack> : <></>}
        {profile.snapchat ? <HStack spacing="4">
          <SnapIcon boxSize="2em" />
          <Box>{profile.snapchat}</Box>
        </HStack> : <></>}
        {profile.twitter ? <HStack spacing="4">
